    ...
    def __init__(self,
        ...
        self.summary = '''策略概念: 券資比越高代表散戶看空，法人買超股票會上漲，這時候賣可以跟大部分散戶進行相反的操作，反之亦然
                          策略規則: 券資比>=30% 且法人買超股票, 賣
                                    券資比<30% 且法人賣超股票 買'''
        self.MarginPurchaseShortSale[['ShortSaleTodayBalance', 'MarginPurchaseTodayBalance']] = \
            self.MarginPurchaseShortSale[['ShortSaleTodayBalance', 'MarginPurchaseTodayBalance']].astype(int)
        self.MarginPurchaseShortSale['ShortSaleMarginPurchaseTodayRatio'] = self.MarginPurchaseShortSale[
                                                                                'ShortSaleTodayBalance'] / \
                                                                            self.MarginPurchaseShortSale[
                                                                                'MarginPurchaseTodayBalance']

        self.InstitutionalInvestorsBuySell[['sell', 'buy']] = self.InstitutionalInvestorsBuySell[
            ['sell', 'buy']].astype(int)
        self.InstitutionalInvestorsBuySell = self.InstitutionalInvestorsBuySell.groupby(['date', 'stock_id'],
                                                                                        as_index=False).agg(
            {'buy': np.sum, 'sell': np.sum})
        self.InstitutionalInvestorsBuySell['diff'] = self.InstitutionalInvestorsBuySell['buy'] - \
                                                     self.InstitutionalInvestorsBuySell['sell']

        self.stock_price = pd.merge(self.stock_price[['stock_id', 'date', 'open', 'close']],
                                    self.InstitutionalInvestorsBuySell[['stock_id', 'date', 'diff']],
                                    on=['stock_id', 'date'], how='left').fillna(0)
        self.stock_price = pd.merge(self.stock_price, self.MarginPurchaseShortSale[
            ['stock_id', 'date', 'ShortSaleMarginPurchaseTodayRatio']], on=['stock_id', 'date'], how='left').fillna(0)
