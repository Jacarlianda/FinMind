...
class Crawler(cbaseclass.Crawler):
    ...
    def create_loop_list(self):
        ...
        for kind in kind_list:
            ...
            tem = re.findall(' data-name="[A-Za-z ]+" data-id="[0-9]+" ',res.text)
            futures_id_list = [ re.findall('[0-9]+',te)[0] for te in tem ]
            data_name_list = [ re.findall('"[A-Za-z ]+"',te)[0].replace('"','') for te in tem ]
    ...
    def crawler(self,loop):# loop = ['49769', 'Brent Oil Futures']
        def get_value(tem):
            ...
            v = [ float( tem[i].attrib['data-real-value'].replace(',','') ) for i in range(1,6) ]
        ...
        st_date,end_date = self.get_st_date(futures_id,data_name), self.get_end_date()
        
        bonds_url = 'https://www.investing.com/instruments/HistoricalDataAjax'
        form_data = {'curr_id': futures_id,
        'header' : header,
        'st_date': st_date,
        'end_date': end_date,
        'interval_sec': 'Daily',
