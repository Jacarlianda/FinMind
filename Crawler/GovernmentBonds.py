...
class Crawler(cbaseclass.Crawler):
    ...
    def create_loop_list(self):
        ...
        def get_country_url():
            ...
            data_country_id = [dci.replace('data-country-id=', '') for dci in data_country_id]
            page = etree.HTML(res.text)
            tem = []
            for dci in data_country_id:
                tem.append(page.xpath('//option[@data-country-id={}]'.format(dci))[0])

            url = ['https://www.investing.com' + te.attrib['value'] for te in tem]
            # G8 and china
            select = ['canada', 'china', 'france', 'germany',
                      'japan', 'russia', 'uk', 'usa', 'italy']
            country_url = []
            for i in range(len(url)):
                tem = url[i].replace('https://www.investing.com/rates-bonds/', '')
    ...
    def crawler(self, loop):  # loop = ['23681', 'Germany 3 Month']
        ...
        st_date, end_date = self.get_st_date(data_name, cid), self.get_end_date()
